...
class TestProteinEncoder(unittest.TestCase):
    ...
    def test_transform(self):
        X = pd.DataFrame([["ACGTGATAGT"], ["ATCTAGATGGTC"]], columns=['sequence'])
        X_transformed = self.transformer.transform(X)
        pd.testing.assert_frame_equal(X_transformed, pd.DataFrame([["ACGTGATAGT", [31, 10946, 10821]],
                                                                   ["ATCTAGATGGTC", [417, 10821, 421, 3797]]],
    ...
    def test_transform_k_equal_1(self):
        ...
        pd.testing.assert_frame_equal(X_transformed, pd.DataFrame([["ACG", [0, 1, 5]],
                                                                   ["GGTC", [5, 5, 16, 1]]],
                                                                  columns=['sequence', 'kmers_encoding']))
